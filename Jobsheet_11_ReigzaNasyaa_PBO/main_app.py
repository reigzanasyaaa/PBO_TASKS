...
def halaman_riwayat(anggaran: AnggaranHarian):
    ...
    # KONFIRMASI
    if st.session_state.get("konfirmasi", False):
        st.warning(
            f"Yakin ingin menghapus transaksi dengan ID {st.session_state.id_hapus} ?"
        )

        if st.button("Konfirmasi Hapus"):
            berhasil = anggaran.hapus_transaksi(
                int(st.session_state.id_hapus)
            )
            ...
            if berhasil:
                ...
                st.cache_data.clear()
                # reset state
                st.session_state.konfirmasi = False
                st.session_state.id_hapus = 1
                # reload halaman
                st.rerun()
